...
def read_answer_from_coach():
    ...
    """
    try:
        file = open("/shared/coach_ans.txt", "r")
...
def wait_for_answer():
    ...
    _ = conn.sendto(b"OK",("coach",coach_port))
...
def send_to_coach(cmd,args):
    ...
    - parse and send an confirmation in cases of 'selection cmds' and a "big string"
     ...
     """
    try:
        write_to_existing_file("/shared/coach_cmd_args.txt",cmd,args)
    except:
        write_to_non_existing_file("/shared/coach_cmd_args.txt",cmd,args)
    wait_for_answer()
